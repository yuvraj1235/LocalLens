...
import type { UIGraph } from "../schema/uiGraph.js";
...
import {
    ...
    shouldRunFullPipeline,
    type ChangeDetectorState,
} from "../adaptive/adaptiveInference.js";
...
let lastGraph: UIGraph | null = null;
...
async function ensureEngines(models: AnalyzeScreenOptions["models"]) {
    ...
    if (ocrEngine && !ocrEngine.isReady) initPromises.push(ocrEngine.initialize());
    if (nerEngine && !nerEngine.isReady) initPromises.push(nerEngine.initialize());
