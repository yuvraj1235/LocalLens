 ...
 */
import { deduplicateBoxes, nextElementId, resetIdCounter } from "./bboxFusion.js";
 ...
 */
function piiBBoxToXYWH(corners) {
...
export function buildUIGraph(inputs) {
    resetIdCounter();
    ...
    //    box types uniformly.
    const piiXYWH = inputs.piiResults.map((p) => ({ bbox: piiBBoxToXYWH(p.bbox), pii: p }));
    ...
    const detectionBoxes = inputs.detections.map((d) => ({
        bbox: d.bbox,
    ...
    const elements = dedupedDetections.map(({ bbox, confidence, payload }) => {
    ...
    const faces = inputs.faces.map((f) => ({
        ...f,
