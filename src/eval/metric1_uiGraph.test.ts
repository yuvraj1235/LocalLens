...
// Re-implement the pure, testable UIGraph builder logic from content.ts
...
function getLabel(el: Element): string | null {
  return (
    ...
    (() => { const id = el.id; if (!id) return null; const lbl = document.querySelector<HTMLLabelElement>(`label[for="${id}"]`); return lbl?.textContent?.trim() ?? null; })() ||
...
function buildElement(el: HTMLElement, id: string): UIElement {
...
function addEl(html: string): HTMLElement {
...
describe("M1B — Label extraction priority", () => {
  ...
  it("<label for='id'> association resolved", () => {
    ...
    const el = document.getElementById("city") as HTMLElement;
