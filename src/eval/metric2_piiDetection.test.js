...
function computeMetrics(perCase) {
    let tp = 0, fp = 0, fn = 0;
...
describe("M2D — fusePII corroboration logic", () => {
    it("corroborates when regex + NER agree on same region → risk upgraded", () => {
        const regex = [{ type: "EMAIL", bbox: [0, 0, 200, 20], confidence: 0.8, risk: "MEDIUM", source: "regex" }];
        const ner = [{ type: "EMAIL", bbox: [5, 0, 195, 20], confidence: 0.7, risk: "MEDIUM", source: "ner" }];
    ...
    it("solo regex candidate stays at base risk (not corroborated)", () => {
        const regex = [{ type: "PHONE", bbox: [0, 0, 150, 20], confidence: 0.7, risk: "MEDIUM", source: "regex" }];
    ...
    it("non-overlapping candidates are NOT merged (IoU < threshold)", () => {
        const a = [{ type: "EMAIL", bbox: [0, 0, 100, 20], confidence: 0.9, risk: "MEDIUM", source: "regex" }];
        const b = [{ type: "PHONE", bbox: [500, 100, 700, 120], confidence: 0.8, risk: "MEDIUM", source: "ner" }];
