...
global.chrome = {
    storage: { local: {
            get: (k, cb) => { if (k === null) {
                const r = {};
                for (const [kk, v] of mockStorage)
                    r[kk] = v;
                cb(r);
                return;
            } const r = {}; if (typeof k === "string")
                r[k] = mockStorage.get(k); cb(r); },
            set: (items, cb) => { for (const [kk, v] of Object.entries(items))
                mockStorage.set(kk, v); cb?.(); },
            remove: (keys, cb) => { if (typeof keys === "string")
                mockStorage.delete(keys);
            else if (Array.isArray(keys))
                keys.forEach((k) => mockStorage.delete(k)); cb?.(); },
        } },
};
// ── DOM UIGraph build simulation (pure JS, no browser APIs) ──────────────────
function simulateBuildUIGraph(elementCount) {
    const ROLE_MAP = {
