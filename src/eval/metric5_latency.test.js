...
// ── Helpers ────────────────────────────────────────────────────────────────────
function makeContext(nElements = 10) {
    return {
...
function addInteractiveElement(id, tag = "button") {
...
beforeEach(() => {
    ...
    global.window = { scrollBy: vi.fn(), location: { href: "" }, innerWidth: 1280, innerHeight: 800 };
});
// ── Tests ──────────────────────────────────────────────────────────────────────
...
describe("M5B — validateAction schema check latency", () => {
    ...
    it("1,000 validations in < 10 ms (batch throughput)", () => {
        ...
        for (let i = 0; i < 1000; i++)
            validateAction(action, { skipDomCheck: true });
        const ms = performance.now() - start;
...
describe("M5E — History accumulation stays O(1) per step", () => {
    it("history never exceeds 10 entries (capped)", { timeout: 20000 }, async () => {
        let seenHistory = [];
        const mockSend = vi.fn().mockImplementation(async (req) => {
        ...
        const maxHistoryLen = Math.max(...mockSend.mock.calls.map((call) => call[0]?.history?.length ?? 0));
