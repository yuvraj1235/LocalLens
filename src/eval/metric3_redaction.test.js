        ...
        local: {
            get: (keys, cb) => {
                ...
                if (typeof keys === "string")
                    r[keys] = mockStorage.get(keys);
                cb(r);
            },
            set: (items, cb) => { for (const [k, v] of Object.entries(items))
                mockStorage.set(k, v); cb?.(); },
            remove: (keys, cb) => {
                if (typeof keys === "string")
...
function applyRedaction(rawLabel, inputType, inputName, userRedactedKeys = []) {
