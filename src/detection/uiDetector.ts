...
import type { ElementType, RawDetection } from "../schema/uiGraph.js";
...
export async function runUiDetection(
  image: ImageData,
  ...
  const feeds: Record<string, ort.Tensor> = { [INPUT_NAME]: tensor };
  const output = await session.run(feeds);

  const raw = output[OUTPUT_NAME];
  if (!raw) {
    throw new Error(
      `Expected output tensor named "${OUTPUT_NAME}" but got: ${Object.keys(output).join(", ")}. ` +
...
// ---------------------------------------------------------------------------
// Preprocessing: letterbox resize + normalize to NCHW float32 [0,1]
// ---------------------------------------------------------------------------
...
function preprocess(image: ImageData, inputSize: number): PreprocessResult {
...
// ---------------------------------------------------------------------------
// Decode: Ultralytics output0 [1, 4+numClasses, numAnchors] -> candidate boxes
// ---------------------------------------------------------------------------

interface Candidate {
  type: ElementType;
  bbox: [number, number, number, number]; // cx, cy, w, h in INPUT_SIZE space
  confidence: number;
}

function decode(output: ort.Tensor, numClasses: number, confidenceThreshold: number): Candidate[] {
...
function nonMaxSuppression(candidates: Candidate[], iouThreshold: number): Candidate[] {
  const byClass = new Map<ElementType, Candidate[]>();
