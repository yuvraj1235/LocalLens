...
function addElement(id: string, tag = "button"): HTMLElement {
...
describe("TYPE", () => {
  it("sets element value and fires input + change events", async () => {
    const el = addElement("inp_email", "input") as HTMLInputElement;
    ...
    const result = await executeAction(
      makeAction({ action: "TYPE", element_id: "inp_email", value: "hello@test.com" })
    );
  ...
  it("returns error when value is null", async () => {
    ...
    const result = await executeAction(
      makeAction({ action: "TYPE", element_id: "inp_1", value: null })
    );
    expect(result.status).toBe("error");
...
describe("SCROLL", () => {
  it("calls window.scrollBy with parsed x,y values", async () => {
    ...
    const result = await executeAction(
      makeAction({ action: "SCROLL", element_id: null, value: "0,500" })
    );
  ...
  it("uses default scroll of 300px when value is null", async () => {
    ...
    const result = await executeAction(
      makeAction({ action: "SCROLL", element_id: null, value: null })
    );
...
describe("SELECT", () => {
  it("sets select value and fires change event", async () => {
    const el = addElement("sel_1", "select") as HTMLSelectElement;
    ...
    const result = await executeAction(
      makeAction({ action: "SELECT", element_id: "sel_1", value: "blue" })
    );
  ...
  it("returns error when value is null", async () => {
    ...
    const result = await executeAction(
      makeAction({ action: "SELECT", element_id: "sel_2", value: null })
    );
    expect(result.status).toBe("error");
    expect(result.message).toMatch(/requires a value/);
  });
...
describe("NAVIGATE", () => {
  it("sets window.location.href to the given URL", async () => {
    ...
    const result = await executeAction(
      makeAction({ action: "NAVIGATE", element_id: null, value: "https://example.com" })
    );
    expect(result.status).toBe("ok");
  ...
  it("returns error when value is null", async () => {
    const result = await executeAction(
      makeAction({ action: "NAVIGATE", element_id: null, value: null })
    );
    expect(result.status).toBe("error");
...
describe("WAIT", () => {
  it("waits for the specified number of ms", async () => {
    vi.useFakeTimers();
    const waitPromise = executeAction(
      makeAction({ action: "WAIT", element_id: null, value: "500" })
    );
  ...
  it("defaults to 1000ms when value is null", async () => {
    vi.useFakeTimers();
    const waitPromise = executeAction(
      makeAction({ action: "WAIT", element_id: null, value: null })
    );
...
describe("Terminal actions", () => {
  it("DONE returns ok status", async () => {
    const result = await executeAction(
      makeAction({ action: "DONE", element_id: null, done: true })
    );
    expect(result.status).toBe("ok");
  ...
  it("ASK_USER returns ok status with the value in message", async () => {
    const result = await executeAction(
      makeAction({ action: "ASK_USER", element_id: null, value: "Which option?" })
    );
    expect(result.status).toBe("ok");
