...
function addElement(id: string, options: {
  tag?: string;
  hidden?: boolean;
  disabled?: boolean;
  zeroSize?: boolean;
  ...
  if (options.hidden)   (el as HTMLElement).style.display = "none";
  if (options.disabled) (el as HTMLInputElement).disabled = true;
...
describe("Schema validation", () => {
  it("rejects an unknown action type", () => {
    const result = validateAction(makeAction({ action: "HOVER" as any }));
