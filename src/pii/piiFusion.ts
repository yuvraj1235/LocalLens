...
export type PIISource = 'regex' | 'ner' | 'dom' | 'vision';

export interface PIICandidate {
  type: PIIType;
  /** Axis-aligned box in source-image pixel coords: [x0, y0, x1, y1]. */
...
/** Final fused output — same shape as PIICandidate but with provenance kept. */
export interface FusedPII extends PIICandidate {
  /** All sources that corroborated this region (deduped). */
  sources: PIISource[];
  /** True if 2+ independent sources overlapped on this region. */
  corroborated: boolean;
}

export type FusedPiiResult = FusedPII;
...
const BASE_RISK: Record<PIIType, RiskLevel> = {
  PASSWORD: 'HIGH',
  CREDIT_CARD: 'HIGH',
  OTP: 'HIGH',
  ID_NUMBER: 'HIGH',
...
function upgradeRisk(risk: RiskLevel): RiskLevel {
...
function iou(a: PIICandidate['bbox'], b: PIICandidate['bbox']): number {
...
function unionBBox(a: PIICandidate['bbox'], b: PIICandidate['bbox']): PIICandidate['bbox'] {
  return [
    Math.min(a[0], b[0]),
    Math.min(a[1], b[1]),
    Math.max(a[2], b[2]),
...
export interface PIIFusionOptions {
  /** Minimum bbox IoU for two candidates to be treated as the same region. */
 ...
 */
export function fusePII(
  regexCandidates: PIICandidate[],
  nerCandidates: PIICandidate[],
  options: PIIFusionOptions = {}
): FusedPII[] {
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    const sources = new Set<PIISource>([all[i].source]);
    let matchCount = 1;

    for (let j = i + 1; j < all.length; j++) {
      if (used[j]) continue;
      if (iou(mergedBBox, all[j].bbox) < overlapThreshold) continue;
