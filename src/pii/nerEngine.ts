...
const ENTITY_TO_PII_TYPE: Record<string, PIIType> = {
  PERSON: 'PERSON',
  LOCATION: 'LOCATION',
...
const ENTITY_BASE_RISK: Record<string, RiskLevel> = {
  PERSON: 'MEDIUM',
  ADDRESS: 'MEDIUM',
  LOCATION: 'LOW',
  ORGANIZATION: 'LOW',
...
interface TokenizedInput {
  tokenIds: number[];
  /** Maps each token index back to a [startChar, endChar) span in the original text. */
...
class WordPieceTokenizer {
  ...
  constructor(vocab: Map<string, number>, maxLen = 128) {
    this.vocab = vocab;
  ...
  static async fromUrl(url: string): Promise<WordPieceTokenizer> {
    const res = await fetch(url);
    if (!res.ok) throw new Error(`nerEngine: failed to load vocab from ${url} (${res.status})`);
    const text = await res.text();
    const vocab = new Map<string, number>();
    text.split('\n').forEach((line, idx) => {
      const token = line.replace(/\r$/, '');
      if (token.length > 0) vocab.set(token, idx);
    });
  ...
  tokenize(text: string): TokenizedInput {
    ...
    const offsets: Array<[number, number]> = [[-1, -1]]; // [CLS] has no span

    for (const [word, start, end] of words) {
      const pieces = this.wordPiece(word.toLowerCase());
      if (tokenIds.length + pieces.length >= this.maxLen - 1) break;
  ...
  private basicSplit(text: string): Array<[string, number, number]> {
...
export class NEREngine {
  ...
  private readonly config: Required<Omit<NEREngineConfig, 'executionProviders'>> &
    Pick<NEREngineConfig, 'executionProviders'>;
  private ready = false;

  constructor(config: NEREngineConfig) {
    this.config = {
  ...
  async detect(ocrResults: OCRResult[]): Promise<PIICandidate[]> {
  ...
  private async detectInLine(line: OCRResult): Promise<PIICandidate[]> {
    ...
    const perTokenLabel: Label[] = [];
    const perTokenConfidence: number[] = [];
  ...
  private decodeEntities(
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      if (offsets[i][0] === -1) continue; // skip [CLS]/[SEP]
...
function softmaxScore(data: Float32Array, offset: number, numLabels: number, idx: number): number {
  let sum = 0;
  const max = data[offset + idx];
  for (let c = 0; c < numLabels; c++) sum += Math.exp(data[offset + c] - max);
