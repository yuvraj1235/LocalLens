 ...
 */
function connectedComponents(binary, probMap, width, height, minArea) {
...
function unclipBox(r, ratio, maxW, maxH) {
    const w = r.x1 - r.x0;
    const h = r.y1 - r.y0;
...
export class OCREngine {
    ...
    async initialize() {
        if (this.ready)
            return;
        const executionProviders = this.config.executionProviders ?? (await this.resolveExecutionProviders());
    ...
    async run(source) {
        ...
        if (!srcCtx)
            throw new Error('OCREngine: unable to acquire 2D context from source canvas.');
    ...
    // Detection stage
    // -------------------------------------------------------------------
    async detectRegions(ctx, srcWidth, srcHeight) {
        ...
        const [, , outH, outW] = probTensor.dims;
        ...
        const rawRegions = connectedComponents(binary, probData, outW, outH, this.config.minRegionArea).filter((r) => r.score >= this.config.dbBoxThresh);
    ...
    imageDataToDetTensor(imageData, resizedW, resizedH) {
    ...
    async recognizeCrop(crop) {
        ...
        const [, seqLen, numClasses] = logits.dims;
        return this.ctcGreedyDecode(data, seqLen, numClasses);
    }
    ctcGreedyDecode(data, seqLen, numClasses) {
        let text = '';
        let lastIndex = -1;
        let probSum = 0;
        let charCount = 0;
    ...
    // Setup helpers
    // -------------------------------------------------------------------
    async resolveExecutionProviders() {
        const hasWebGPU = typeof navigator !== 'undefined' && 'gpu' in navigator;
        return hasWebGPU ? ['webgpu', 'wasm'] : ['wasm'];
    }
    async loadCharDict(url) {
