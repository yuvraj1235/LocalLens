...
function b642ab(b64: string): ArrayBuffer {
  ...
  for (let i = 0; i < clean.length; i += 4) {
    ...
    bytes[bi++] = (v0 << 2) | (v1 >> 4);
    if (clean[i + 2]) bytes[bi++] = ((v1 & 15) << 4) | (v2 >> 2);
    if (clean[i + 3]) bytes[bi++] = ((v2 & 3) << 6) | v3;
  }
  return bytes.buffer;
}
...
function storageGet(keys: string | string[] | null): Promise<any> {
...
async function getMasterKey(): Promise<CryptoKey> {
  ...
  if (storage[KEY_STORAGE_ID]) {
    // Import existing key
    masterKeyCache = await webcrypto.subtle.importKey(
      "jwk",
      storage[KEY_STORAGE_ID],
      { name: "AES-GCM" },
      true,
      ["encrypt", "decrypt"]
    );
    ...
    masterKeyCache = await webcrypto.subtle.generateKey(
...
export async function listCachedKeys(): Promise<CachedEntry[]> {
  ...
  for (const [k, v] of Object.entries(allData)) {
    if (k.startsWith(CACHE_PREFIX)) {
      ...
      const cacheData = v as { key: string; updatedAt: number; ciphertext: string; iv: string };
...
export async function clearAllCachedValues(): Promise<void> {
  const allData = await storageGet(null);
...
export async function updateAutofillSettings(updates: Partial<AutofillSettings>): Promise<void> {
